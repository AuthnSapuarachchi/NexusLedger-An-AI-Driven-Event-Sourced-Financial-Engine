...
const Dashboard = () => {
  ...
  const handleTransfer = async (e) => {
    ...
    // EXPLICIT PAYLOAD: This ensures we know exactly what is being sent
    const payload = {
        fromId: formData.fromId.trim(), // Remove accidental spaces
        toId: formData.toId.trim(),
        amount: parseFloat(formData.amount) // Ensure it's a number, not a string
    };
    ...
    try {
        ...
        // Use the explicit payload here too
        const newTx = { 
            id: idKey, 
            ...payload, 
            status: 'QUEUED', 
        ...
        alert("Error: " + (err.response?.data?.message || "Invalid Data Format"));
    } finally {
        setSubmitting(false);
    }
};
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white p-8 font-sans">
      ...
      <div className="mb-8 bg-gradient-to-r from-green-900 to-green-800 p-6 rounded-xl border border-green-700">
      ...
      </div>assName="flex gap-4- Auto-filled with logged-in user's account */}
              <input 
        ...
        <section className="bg-slate-800 p-6 rounded-xl border border-slate-700 h-fit">
          ...
          <form onSubmit={handleTransfer} className="space-x-1 space-y-4">
              ...
              >
                {submittName="w-full bg-slate-900 border border-slate-700 p-3 rounded"
